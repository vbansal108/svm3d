...
public class SVM {
	...
	public XValResult xval(Vector<svm_node[]> vx, Vector<Double> vy, Parameters parameters, int nrfold, Range cR, int cS, Range gR, int gS){
		this.parameters = parameters;
		this.param = parameters.getParam();
		
		load(vx, vy);

		double[][] errors = new double[cS][gS];
		double cStep = cR.getRange() / cS;
		double gStep = gR.getRange() / gS;
		double bestC = 0;
		double bestG = 0;
		...
		for (int i = 0; i < cS; i++) {
			for (int j = 0; j < gS; j++) {
				param.C = cR.getMin() + cStep * i;
				param.gamma = gR.getMin() + gStep * j;
